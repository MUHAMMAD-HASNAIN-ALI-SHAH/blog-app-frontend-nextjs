...
const Blog = () => {
  ...
  const { like, getBlogData, blog, viewBlog } = useHomeBlogStore();
  const { isAuthenticated } = useAuthStore();
  const [loading, setLoading] = useState(false);
  const hasViewedRef = useRef(false);

  if (!blogId) {
    return <p className="text-center text-red-500 font-medium mt-10">Error: Invalid blog ID</p>;
  }
        ...
        {blog ? (
          <>
            ...
            <div className="flex flex-col items-center justify-center gap-1 mb-6">
              ...
              <p className="text-gray-600 font-medium">{blog.likes?.length || 0} Likes</p>
            </div>
